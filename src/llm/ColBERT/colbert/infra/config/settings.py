...
from src.llm.ColBERT.colbert.utils.utils import timestamp
...
class RunSettings:
    ...
    def index_root_(self):
        return self.index_root or os.path.join(self.root, self.experiment, f'indexes{os.sep}')
    ...
    def script_name_(self):
        if '__file__' in dir(__main__):
            ...
            script_name = script_path.replace(f'{os.sep}', '.').strip('.')[:-3]
