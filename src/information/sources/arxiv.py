...
from src.information.sources.base import ContentSearchEngine, requires_valid_period, InformationSource
...
class ArxivSearchEngine(ContentSearchEngine):
    ...
    def __init__(self):
        ...
        self.max_results = None  # Maximum number of results to fetch from the query
        self.pdf_extractor_provider = None  # Provider for extracting PDF content
    ...
    def extract_from_xmls(self, xml):
        ...
        try:
            ...
            entries = root.findall(f".//{namespace}entry")  # Find all entry nodes
            arxivs = []
            for entry in entries:
                ...
                authors = list(
                    map(lambda x: x.find(f".//{namespace}name").text, entry.findall(f".//{namespace}author")))
            ...
            self.logger.error("Error parsing XML: %s", e)  # Log XML parsing errors
            return []
    ...
    def search(self, save_callback=None, stop_event: threading.Event = None) -> list:
        ...
        categories = "(" + "+OR+".join(self.topics) + ")"  # Build category query
        to_date = datetime.datetime.now()
        from_date = to_date - datetime.timedelta(days=self.period)  # Calculate the date range
        ...
        try:
            response = requests.get(url, timeout=self._REQUEST_TIMEOUT)  # Perform the HTTP GET request
            if response.status_code != 200:
                self.logger.error("Failed to fetch data from URL: %s, Status Code: %s", url, response.status_code)
                return []
            arxivs = self.extract_from_xmls(response.text)  # Extract metadata from XML response
            results = []
            for arxiv in arxivs:

                if stop_event and stop_event.is_set():
                    self.logger.info("Stop event called in the middle of procesing a pdf")
                    break

                arxiv = self.extract_pdf_info(arxiv)  # Process the PDF content for each result
                if arxiv:
                    results.append(arxiv)
                    if save_callback:
                        self.save_if_valid(save_callback, arxiv)  # Save valid results using the callback
            return results
    ...
    def extract_pdf_info(self, arxiv):
        ...
        try:
            response = requests.get(url, timeout=self._REQUEST_TIMEOUT)  # Fetch the PDF content
            if response.status_code != 200:
                self.logger.error("Failed to fetch PDF: %s, Status Code: %s", url, response.status_code)
                return None
            extractor = PDFExtractorProvider.build(self.pdf_extractor_provider)  # Initialize PDF extractor
            text = extractor.extract(response.content)  # Extract text from the PDF
            ...
            with DocumentRetrieverProvider.get_document_retriever_provider(self.provider,
                                                                           document_name=title,
                                                                           logger=self.logger) as provider:
                important_paragraphs, _ = provider.search([text])  # Retrieve important paragraphs
                if important_paragraphs:
                    res = "\n".join(important_paragraphs)  # Join paragraphs into a single string
                    arxiv["content"] = res  # Add the extracted content to the metadata
                    arxiv["information_source"] = self.information_source.value
                    arxiv["extracted_images"]: extractor.extracted_images  # Extracted raw images if used docling engine

                else:
                    self.logger.warning("No important paragraphs found for title: %s", title)
        except Exception as e:
            self.logger.error("Error during PDF extraction: %s. URL: %s", e, url)
            arxiv = None
        finally:
            return arxiv
