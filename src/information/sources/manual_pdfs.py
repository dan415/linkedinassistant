...
class ManualSourceEngine(ContentSearchEngine):
    ...
    def __init__(self):
        ...
        self.pdf_manager = B2Handler()  # Manager for interacting with the B2 storage
    ...
    def search(self, save_callback=None, stop_event: threading.Event = None) -> list:
        ...
        try:
            ...
            files = self.pdf_manager.list_folder_contents(FileManagedFolders.INPUT_PDF_FOLDER)
            ...
            for pdf in pdfs:
                ...
                pdf_content = self.pdf_manager.download(f"{FileManagedFolders.INPUT_PDF_FOLDER}/{pdf['name']}")
                if pdf_content:
                    extracted_content = self.extract_pdf_info(pdf_content)
                    if extracted_content:
                        res.append(extracted_content)
                        if save_callback:
                            self.save_if_valid(save_callback,
                                               extracted_content)  # Save valid content using the callback
                    # Move processed PDF to the output folder
                    source_path = f"{FileManagedFolders.INPUT_PDF_FOLDER}/{pdf['name']}"
                    dest_path = f"{FileManagedFolders.OUTPUT_PDF_FOLDER}/{pdf['name']}"
                    try:
    ...
    def extract_pdf_info(self, pdf_content: bytes):
        ...
        if self.provider is None:
            raise ValueError("Document Retrieval Provider must be specified beforehand")
        ...
        try:
            ...
            text = extractor.extract(pdf_content)  # Extract text from the PDF content
            ...
            with DocumentRetrieverProvider.get_document_retriever_provider(self.provider,
                ...
                important_paragraphs, extracted_title = provider.search([text])  # Search for relevant paragraphs
